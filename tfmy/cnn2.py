...
if __name__ == '__main__':
    ...
    # CNN
    yw_train = y_wind[:, 0]  # price
    xw_train = x_wind[:train_st]  # (2, 499)
    yw_train = yw_train[:train_st]  # (2, 499)
    ...
    def run_optimization(x, y):
        ...
        with tf.GradientTape() as g:
            # Forward pass.
            pred = cnn_net(x, is_training=True)
            # Compute loss.
            loss = my_loss(pred, y)

        # Variables to update, i.e. trainable variables.
        trainable_variables = cnn_net.trainable_variables
    ...
    for i in range(STEPS):
        for step, (batch_x, batch_y) in enumerate(train_data_w.take(-1), 1):  # (batch, steps, inputs)
