...
if __name__ == '__main__':
    ...
    x_wind = np.array(l['x'], dtype=np.float32)  # steps, window_size, features
    y_wind = np.array(l['y'], dtype=np.float32)  # steps, features
    x_wind_least = np.array(l['x_without_y'], dtype=np.float32)  # steps, window_size, features
    w_size = np.array(l['w_size'], dtype=np.float32)
    future = np.array(l['future'], dtype=np.float32)
    ...
    # optimizer = tf.optimizers.Adam(learning_rate=0.00000551)
    optimizer = tf.optimizers.Adam(learning_rate=0.00110251)
    ...
    def run_optimization(x, y, step):
        ...
        with tf.GradientTape() as tape:
            ...
            # Compute loss.
            mnog = step / (len(xw_train) + 1)
            # loss = tf.keras.losses.MSLE(y, pred_u[:, 0])
            loss = tf.keras.losses.MAPE(y, pred_u[:, 0])
            loss = tf.reduce_mean(loss) * mnog
    ...
    for i in range(STEPS):
        ...
        for step, (batch_x, batch_y) in enumerate(train_data_w.take(-1)):  # (batch, steps, inputs)
            ...
            # print("step", step)
            # print(batch_x.shape)
            loss = run_optimization(batch_x, batch_y, step)
