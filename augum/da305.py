...
def main():
    ...
    mcxsm = ''
    p = '/home/u2/Downloads/CHMF_140522_200310.txt'
    lim = 0
    data_full = augum(p, [7, 8, 3, 4, 5, 6])[:800]  # steps, price/volume/etc..
    ...
    data_full[:, 0] = scaler_sections(data_full[:, 0], div)

    data_full_orig = data_full.copy()
    # SMOOTH
    for d in range(data_full.shape[1]):
        # data_full[:, d] = savitzky_golay(data_full[:, d], window_size=31, order=0.5)
        data_full[:, d] = savitzky_golay(data_full[:, d], window_size=21, order=9.5)
    #
